...
Website ôn tập trắc nghiệm **CNS/ATM** (Communication · Navigation · Surveillance) dành cho nhân viên kỹ thuật điện tử hàng không (ATSEP) tại **Trung tâm Kiểm soát đường dài Hồ Chí Minh - VATMSORATS Long Thành ATCC**.
...
├── admin.html              # Dashboard analytics + Team Readiness
├── check.html              # Công cụ kiểm tra ngân hàng đề
├── css/
│   └── style.css           # Stylesheet tùy chỉnh
├── js/
│   ├── questions.js        # ⚠️ Database câu hỏi (1.858 câu)
│   └── app.js              # Logic quiz + webhook telemetry
├── apps-script/
│   └── webhook.gs          # Google Apps Script — nhận & lưu telemetry vào Sheet
└── README.md
```
...
Dashboard theo dõi năng lực đội ngũ **ẩn danh** tích hợp trong `admin.html`.

### Cách hoạt động
Mỗi khi ATSEP trả lời câu hỏi (ôn tập hoặc thi thử), hệ thống tự động ghi nhận 4 trường dữ liệu vào Google Sheet qua Apps Script webhook:

| Trường | Mô tả |
|---|---|
| `module` | Module chuyên ngành (VHF, VCCS, ADS-B, ...) |
| `viTri` | Vị trí thi (Long Thành / Tân Sơn Nhất) |
| `questionId` | ID câu hỏi |
| `isWrong` | Đúng hay sai |

> 🔒 **Hoàn toàn ẩn danh** — không gửi tên, chức danh, hay bất kỳ thông tin định danh cá nhân nào.

### Tính năng dashboard
- **Module Mastery**: % thành thạo từng module (đỏ <60% · vàng 60–79% · xanh ≥80%)
- **Top câu sai nhiều nhất**: Xác định câu hỏi cần tập trung ôn luyện (ngưỡng tối thiểu 3 lượt)
- Dữ liệu cập nhật real-time, có nút "Làm mới"

### Cấu hình (dành cho admin)
1. Deploy `apps-script/webhook.gs` lên Google Apps Script (Execute as Me · Anyone)
2. Điền URL `/exec` vào `TEAM_WEBHOOK_URL` trong `js/app.js`
3. Dashboard tự nhận dữ liệu từ toàn đội ngay sau khi cấu hình
...
|---|---|---|
| v1.4-2026 | 18/06/2026 | Team Readiness Dashboard · Webhook telemetry ẩn danh · apps-script/webhook.gs |
